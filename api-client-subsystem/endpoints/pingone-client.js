    ...
    if (!response.ok) {
      ...
      if (verbose) {
        try {
          ...
          try { importLogger.warn('[PingOne] Verbose error diagnostics for createUser', { url, populationId, status: response.status, statusText: response.statusText, responseHeaders: respHeaders }); } catch { /* ignore import logger errors */ }
        } catch { /* ignore verbose logging errors */ }
      }
      ...
      if (verbose) {
        try {
          ...
          try { importLogger.warn('[PingOne] Verbose error body for createUser', { url, populationId, status: response.status, bodyPreview: preview }); } catch { /* ignore import logger errors */ }
        } catch { /* ignore verbose logging errors */ }
      }
