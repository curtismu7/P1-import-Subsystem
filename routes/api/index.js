/**
 * @file Main API routes for the PingOne Import Tool.
...
// Modify routes (static field update across a population)
const modifyRouter = express.Router();
modifyRouter.post('/static-update', express.json(), async (req, res) => {
  try {
    const { populationId, populationName, fieldKey, type = 'string', value } = req.body || {};
    if (!populationId || !fieldKey) {
      return res.error('populationId and fieldKey are required', { code: 'MISSING_FIELDS' }, 400);
    }

    // Resolve token/env
    const tokenManager = req.app.get('tokenManager');
    if (!tokenManager) { return res.error('Token manager not available', { code: 'NO_TOKEN_MANAGER' }, 500); }
    const token = await tokenManager.getAccessToken();
    if (!token) { return res.error('Failed to get access token', { code: 'NO_TOKEN' }, 401); }
    const environmentId = await tokenManager.getEnvironmentId();
    if (!environmentId) { return res.error('Environment ID not configured', { code: 'NO_ENV' }, 400); }
    const apiBaseUrl = tokenManager.getApiBaseUrl();

    // Helper: build nested payload from dot path
    const buildPatchBody = (path, raw) => {
      const val = type === 'boolean' ? String(raw).toLowerCase() === 'true' : raw;
      const parts = String(path).split('.');
      const root = {};
      let cur = root;
      for (let i = 0; i < parts.length; i++) {
        const key = parts[i];
        if (i === parts.length - 1) { cur[key] = val; }
    ...
    // Prebuild path
    const patchTemplatePath = fieldKey;

    while (true) {
      let url = `${apiBaseUrl}/environments/${environmentId}/users?population.id=${encodeURIComponent(populationId)}&limit=${limit}&offset=${offset}`;
      url += '&expand=population';
      const r = await fetch(url, { headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' } });
      if (!r.ok) {
        const t = await r.text();
        return res.error('Failed to list users for static update', { status: r.status, details: t }, r.status);
      }
      ...
      for (const u of users) {
        try {
          const patchBody = buildPatchBody(patchTemplatePath, value);
          const pr = await fetch(`${apiBaseUrl}/environments/${environmentId}/users/${u.id}`, {
            method: 'PATCH',
            headers: {
          ...
          if (!pr.ok) {
            const te = await pr.text();
            totalFailed++;
            errors.push({ id: u.id, username: u.username, status: pr.status, error: te });
          } else {
            totalUpdated++;
          }
          // Gentle rate-limit
          ...
          totalFailed++;
          errors.push({ id: u.id, username: u.username, error: e.message });
        }
      }
    ...
    try {
      await logOperationToHistory(req, {
        type: 'MODIFY',
        populationName: populationName || populationId,
        message: `Static update: set ${fieldKey} to "${value}" on ${totalUpdated} users`,
    ...
    return res.success('Static update completed', {
      updated: totalUpdated,
      failed: totalFailed,
      errors: errors.slice(0, 25) // cap to keep response small
    });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    // Back-compat for dashboard: map recent import-shaped entries to data.imports
    try {
      const imports = paginatedHistory
        .filter(it => (it.type && String(it.type).toUpperCase().includes('IMPORT')) || it.jobName || it.records_processed || it.total)
        .map(it => ({
          job_name: it.jobName || it.message || 'Import',
