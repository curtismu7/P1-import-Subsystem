...
// Append a normalized entry to logs/operation-history.json for the dashboard
async function appendOperationHistory(entry) {
  try {
    ensureLogsDir();
    const historyFile = path.join(process.cwd(), 'logs', 'operation-history.json');
    let history = [];
    try {
...
router.post('/', upload.single('file'), async (req, res) => {
  try {
    ...
    // Log import start (include populationId for traceability)
    ...
    importLog('Import started', { fileName, totalRecords, sessionId, populationId: req.body.populationId });
...
async function runRealImportWorker({ sessionId, populationId, csvContent, sendWelcome, realtimeManager }) {
  ...
  let skipped = 0;
  const createdEmails = [];
  // Sample collections for UI diagnostics (kept small)
  const errorSamples = [];
  const skipSamples = [];
  ...
  const processOne = async (row) => {
    ...
    try {
      ...
      // Robust error parsing: prefer structured JSON error codes over substring matches
      let parsed, errorCode = null, errorMessage = '';
      try { parsed = JSON.parse(bodyStr); } catch (_) { parsed = null; }
      if (parsed && typeof parsed === 'object') {
        errorCode = parsed?.error?.code || parsed?.code || (Array.isArray(parsed?.details) ? parsed.details[0]?.code : null);
        errorMessage = parsed?.error?.message || parsed?.message || bodyStr;
      } else {
        errorMessage = bodyStr;
      }
      const isUniquenessViolation = /UNIQUENESS_VIOLATION/i.test(String(errorCode));
      const isDuplicateMsg = /duplicate|already exists/i.test(String(errorMessage));
      // Treat as duplicate only when HTTP 409 with duplicate semantics OR explicit code says UNIQUENESS_VIOLATION
      const isDuplicateStatus = (status === 409 && (isDuplicateMsg || isUniquenessViolation)) || isUniquenessViolation;
      if (isDuplicateStatus) {
        // Verify duplicate strictly within the target population before skipping
        let verifiedDuplicateInPopulation = false;
        try {
          const baseUrl = client?.config?.baseUrl || 'https://api.pingone.com/v1';
          const lookupUrl = `${baseUrl}/environments/${environmentId}/populations/${populationId}/users?filter=username%20eq%20"${encodeURIComponent(userPayload.username)}"`;
          const lookupResp = await fetch(lookupUrl, { headers: { 'Authorization': `Bearer ${await client.tokenManager.getAccessToken()}` } });
          if (lookupResp.ok) {
            const lookupJson = await lookupResp.json();
            const hasUser = Array.isArray(lookupJson?._embedded?.users) && lookupJson._embedded.users.length > 0;
            verifiedDuplicateInPopulation = hasUser;
          }
        } catch (_) { /* non-blocking */ }

        if (verifiedDuplicateInPopulation) {
          skipped++;
          const snippet = (bodyStr.slice(0, 180) || '').replace(/\s+/g, ' ');
          const reason = isUniquenessViolation ? 'UNIQUENESS_VIOLATION' : (isDuplicateMsg ? 'already_exists' : 'conflict');
          console.info('Duplicate detected in population, skipping user', { status, reason, snippet });
          importLog('Duplicate detected in population, skipping user', { status, reason, snippet, username: userPayload.username, email: userPayload.email });
          // Sample capture for UI
          try {
            if (skipSamples.length < SAMPLE_LIMIT) {
              skipSamples.push({ reason, username: userPayload.username, email: userPayload.email });
            }
          } catch (_) {}
        } else {
          // Treat as failure so the UI reflects that users were not created due to env-level conflicts
          failed++;
          importStatus.errors++;
          const snippet = (bodyStr.slice(0, 500) || '').replace(/\s+/g, ' ');
          console.warn('Create user conflict (not in population) counted as failure', { status, code: errorCode, message: err?.message, snippet });
          try { importLogger.warn('Create user conflict (not in population)', { status, code: errorCode, message: errorMessage || err?.message, snippet, username: userPayload.username, email: userPayload.email, populationId, sessionId }); } catch (_) {}
          importLog('Create user conflict (counted as failure)', { status, message: errorMessage || err?.message, snippet, username: userPayload.username, email: userPayload.email });
          // Sample capture for UI
          try {
            if (errorSamples.length < SAMPLE_LIMIT) {
              errorSamples.push({ code: errorCode, status, message: errorMessage || err?.message || 'Create failed' });
            }
          } catch (_) {}
        }
      } else {
        ...
        importStatus.errors++;
        const snippet = (bodyStr.slice(0, 500) || '').replace(/\s+/g, ' ');
        console.warn('Create user failed', { status, code: errorCode, message: errorMessage || err?.message, snippet });
        try {
          importLogger.warn('Create user failed', { status, code: errorCode, message: errorMessage || err?.message, snippet, username: userPayload.username, email: userPayload.email, populationId, sessionId });
        } catch (_) {}
        importLog('Create user failed', { status, message: errorMessage || err?.message, snippet, username: userPayload.username, email: userPayload.email });
        if (status === 429) {
  ...
  try {
    importLogger.info('🔚 Import completed', { processed: importStatus.processed, created, skipped, failed, durationMs: importStatus.endTime - importStatus.startTime, populationId, separator: logSeparator('═', 80) });
    importLog('Import completed', { processed: importStatus.processed, created, skipped, failed, durationMs: importStatus.endTime - importStatus.startTime, populationId });
  } catch (_) {}
  ...
  // Persist a history record for the Dashboard "Recent Imports"
  try {
    const historyEntry = {
      id: `imp_${Date.now()}`,
      type: 'IMPORT',
      ...
      message: `Imported ${created} users (${skipped} skipped, ${failed} failed)`,
      success: created,
      errors: failed,
      skipped,
      total: totalRecords
    };
    await appendOperationHistory(historyEntry);
  } catch (_) { /* no-op */ }
