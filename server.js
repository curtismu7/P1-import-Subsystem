 ...
 *
 * @file Main server entry point for PingOne Import Tool
...
app.use(express.static(path.join(__dirname, 'public'), {
  ...
  setHeaders: (res, filePath) => {
    ...
    if (fileExt === '.html') {
      ...
      // Ensure CSS updates are picked up immediately (prevents stale UI)
  ...
  try {
    ...
    server.listen(process.env.PORT || 4000, async () => {
      ...
      try {
        ...
        // Print browser cache-bust helper script for convenience (also log to server.log)
        try {
          const browserCacheScript = '(() => {\n  (async () => {\n    try {\n      // Storage\n      localStorage.clear();\n      sessionStorage.clear();\n\n      // Cache Storage\n      if (\'caches\' in window) {\n        const keys = await caches.keys();\n        await Promise.all(keys.map(k => caches.delete(k)));\n      }\n\n      // Service Workers\n      if (\'serviceWorker\' in navigator) {\n        const regs = await navigator.serviceWorker.getRegistrations();\n        await Promise.all(regs.map(r => r.unregister().catch(() => {})));\n      }\n\n      // IndexedDB (best-effort)\n      if (window.indexedDB) {\n        const list = (indexedDB.databases ? await indexedDB.databases() : []) || [];\n        await Promise.all(\n          list\n            .filter(db => db && db.name)\n            .map(db => new Promise(res => {\n              const req = indexedDB.deleteDatabase(db.name);\n              req.onsuccess = req.onerror = req.onblocked = () => res();\n            }))\n        );\n      }\n\n      console.log(\'✅ Client-side caches cleared. Reloading…\');\n      location.reload(true);\n    } catch (e) {\n      console.error(\'❌ Cache clear failed:\', e);\n    }\n  })();\n})();';
          console.log('\n💡 Run the browser script to clear client-side caches (copy/paste into DevTools Console):\n');
          console.log(browserCacheScript + '\n');
          logger.info('📋 Browser cache-clear script (copy/paste into DevTools Console)', { script: browserCacheScript });

          // Also print a compact one-liner for easy copy/paste
          ...
          console.log('🧪 Quick console one-liner (copy/paste into DevTools Console):');
          console.log(oneLiner + '\n');
          console.log(`📄 Full script file (if generated by restart task): ${process.cwd()}/browser-cache-bust.js`);
        } catch (_) { /* no-op */ }

        // 🚀 INITIALIZE STARTUP SEQUENCE (Token + Populations)
        try {
          logger.info('🚀 Initializing startup sequence...');

          // Import and initialize startup initializer
          const { default: StartupInitializer } = await import('./server/services/startup-initializer.js');
          const startupInitializer = new StartupInitializer();
          ...
          const startupResult = await startupInitializer.runStartupInitialization();
          if (startupResult.success) {
            logger.info('✅ Startup initialization completed successfully');
            console.log('🚀 STARTUP: READY - Token and populations initialized');

            if (startupResult.populations?.success) {
              console.log(`🏘️ POPULATIONS: ${startupResult.populations.populations?.length || 0} loaded and cached`);
            }
            if (startupResult.token?.success) {
              console.log('🔑 TOKEN: Valid access token available');
            }
          } else {
            logger.warn('⚠️ Startup initialization failed - will use API fallback');
            console.log('⚠️ STARTUP: FALLBACK MODE - Using API calls for data');
          }

        } catch (error) {
          logger.error('❌ Error in startup initialization:', error);
          console.log('❌ STARTUP: ERROR - Using API fallback mode');
        }

        // 🗃️ INITIALIZE POPULATION CACHE SERVICE (Legacy - kept for compatibility)
        try {
