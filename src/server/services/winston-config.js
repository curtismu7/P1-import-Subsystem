...
// Get the project root directory (3 levels up from src/server/services/)
...
export function createWinstonLogger(options = {}) {
      ...
      new winston.transports.Console({
        format: winston.format.combine(
          winston.format.printf(({ timestamp, level, message, service, ...meta }) => {
            ...
            const formatted = formatLogLine({ timestamp, level, message, service, tag, separator, ...meta });
  ...
  if (env === 'production') {
    ...
    logger.add(new winston.transports.File({
      filename: path.join(logsDir, 'production-errors.log'),
      level: 'error',
      maxsize: 5 * 1024 * 1024, // 5MB
      maxFiles: 5,
      tailable: true,
...
// Create a server-specific logger that writes to server.log
export function createServerLogger() {
  const logger = createWinstonLogger({ service: 'pingone-import-server' });

  // Add server-specific file transport
  logger.add(new winston.transports.File({
    filename: path.join(logsDir, 'server.log'),
    level: 'info',
    maxsize: 10 * 1024 * 1024, // 10MB
