      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          if (statusContainer && settings.connectionStatus) {
            ...
            const type = settings.connectionStatus === 'connected' ? 'success' : (settings.connectionStatus === 'error' ? 'error' : 'info');
            const icon = type === 'success' ? '✅' : (type === 'error' ? '❌' : 'ℹ️');
            const bgClass = `status-message status-${type}`;
            const message = settings.connectionMessage || 'Connection status unknown';
