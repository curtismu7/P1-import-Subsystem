...
class PingOneApp {
  ...
  async loadVersion() {
    try {
      const resp = await window.csrfManager.fetchWithCSRF('/api/version');
  ...
  async loadSettings() {
    try {
      const response = await window.csrfManager.fetchWithCSRF('/api/settings');
  ...
  async attemptAutoRefresh(reason = 'unknown') {
    try {
      ...
      const resp = await window.csrfManager.fetchWithCSRF('/api/token/refresh', {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  setupEventListeners() {
    ...
    // Make token-info draggable
    this.setupDraggableTokenInfo();
  }
  ...
  shouldShowCredentialsModal() {
    ...
    const hasEnv = Boolean(this.settings.pingone_environment_id);
    const hasClientId = Boolean(this.settings.pingone_client_id);
    const hasSecret = Boolean(this.settings.pingone_client_secret);
  ...
  populateCredentialsForm() {
    ...
    (async () => {
      try {
        const resp = await window.csrfManager.fetchWithCSRF('/api/settings/credentials');
  ...
  async loadCredentialsPopulations() {
    ...
    try {
      const resp = await window.csrfManager.fetchWithCSRF('/api/populations');
  ...
  updateResponsiveLayout() {
    ...
    } else if (leftNav) { leftNav.classList.remove('mobile', 'expanded'); }
  }
  ...
  updateTokenUI() {
    ...
    console.log('🔍 Header elements found:', {
      indicator: Boolean(headerTokenIndicator),
      text: Boolean(headerTokenText),
      time: Boolean(headerTokenTime),
    });
    ...
    if (headerRefreshBtn) { headerRefreshBtn.disabled = Boolean(this.tokenStatus.isRefreshing); }
    if (headerRefreshIcon) {
      if (this.tokenStatus.isRefreshing) { headerRefreshIcon.classList.add('spinning'); } else { headerRefreshIcon.classList.remove('spinning'); }
    }
    ...
    console.log('🔄 Token UI updated:', {
      ...
      headerUpdated: Boolean(headerTokenIndicator),
      footerUpdated: Boolean(footerTokenIndicator),
      homeUpdated: Boolean(homeTokenIndicator),
    });
  ...
  startTokenMonitoring() {
    ...
    // Update server health data every 60 seconds
    setInterval(async () => {
      await this.updateServerHealthData();
    }, 60000);
  ...
  async updateStartupStatusMessage() {
    // Token part
    const tokenValid = Boolean(this.tokenStatus && this.tokenStatus.isValid);
  ...
  /**
   * Update the navigation status box with current system information
   * Reuses existing data from startup and API calls, plus server health data
   */
  updateNavigationStatusBox() {
    try {
      // Token Status - from existing this.tokenStatus
      const tokenStatusEl = document.getElementById('nav-token-status');

      if (tokenStatusEl) {
        if (this.tokenStatus && this.tokenStatus.isValid) {
          tokenStatusEl.textContent = 'Valid';
          tokenStatusEl.className = 'status-value status-success';
        } else if (this.tokenStatus && this.tokenStatus.isRefreshing) {
      ...
      // Token Time - from existing this.tokenStatus.timeLeft
      const tokenTimeEl = document.getElementById('nav-token-time');

      if (tokenTimeEl) {
        if (this.tokenStatus && this.tokenStatus.timeLeft && this.tokenStatus.timeLeft > 0) {
          tokenTimeEl.textContent = this.formatTimeLeft(this.tokenStatus.timeLeft);
        } else if (this.tokenStatus && this.tokenStatus.isRefreshing) {
          tokenTimeEl.textContent = '--:--:--';
        } else {
          tokenTimeEl.textContent = 'Expired';
        }
      }

      // Server Status - always Online since we're running
      const serverStatusEl = document.getElementById('nav-server-status');
      ...
      // Server Health - fetch from /api/health endpoint
      this.updateServerHealthData();

      // Server Start Time - from existing status message (set during startup)
      const serverStartEl = document.getElementById('nav-server-start');

      if (serverStartEl) {
        // Get server start time from existing status message (set by setDefaultStatusBar)
        ...
        if (statusText && statusText.textContent.includes('Server started at:')) {
          ...
          if (startTimeMatch) {
            serverStartEl.textContent = startTimeMatch[1];
          }
        } else {
          // Fallback to current time if status message not set yet
          serverStartEl.textContent = new Date().toLocaleString();
        }
      }

      // Version - from existing this.version (loaded during startup)
      const versionEl = document.getElementById('nav-version');

      if (versionEl) {
        versionEl.textContent = this.version || 'v...';
      }

      // UI Version - already set in HTML, no need to update
      // This comes from the cache-busted CSS/JS files
    } catch (error) {
      console.warn('Failed to update navigation status box:', error);
    }
  }

  /**
   * Update server health data in the navigation status box
   * Fetches data from /api/health endpoint and updates the display
   */
  async updateServerHealthData() {
    try {
      const healthData = await this.getServerHealthData();

      if (!healthData) { return; }

      // Update Server Health status
      const serverHealthEl = document.getElementById('nav-server-health');
      ...
      // Update Memory Usage
      const memoryUsageEl = document.getElementById('nav-memory-usage');

      if (memoryUsageEl && healthData.system && healthData.system.memoryUsage) {
      ...
      if (uptimeEl && healthData.uptime) {
        ...
        uptimeEl.className = 'status-value';
      }
    } catch (error) {
      console.warn('Failed to update server health data:', error);
      ...
      if (memoryUsageEl) {
        ...
        memoryUsageEl.className = 'status-value';
      }
  ...
  /**
   * Get enhanced server health data from /api/health endpoint
   */
  async getServerHealthData() {
    try {
      const response = await window.csrfManager.fetchWithCSRF('/api/health');
      ...
      console.warn('Could not fetch server health data:', error);
    }
  ...
  /**
   * Setup draggable functionality for the token-info element
   */
  setupDraggableTokenInfo() {
    ...
    // Add draggable cursor and make it look interactive
    tokenInfo.style.cursor = 'grab';
    tokenInfo.style.userSelect = 'none';
    tokenInfo.style.position = 'relative';
    tokenInfo.style.zIndex = '1000';

    // Mouse events
    tokenInfo.addEventListener('mousedown', (e) => {
      isDragging = true;
      startX = e.clientX;
      startY = e.clientY;
    ...
    const savedPosition = localStorage.getItem('tokenInfoPosition');
    if (savedPosition) {
      try {
        const position = JSON.parse(savedPosition);
        tokenInfo.style.position = 'fixed';
        tokenInfo.style.left = `${position.left}px`;
        tokenInfo.style.top = `${position.top}px`;
      } catch (e) {
        console.warn('Failed to restore token info position:', e);
      }
    }

    // Add double-click to reset position
    tokenInfo.addEventListener('dblclick', () => {
      localStorage.removeItem('tokenInfoPosition');
      tokenInfo.style.position = 'relative';
      tokenInfo.style.left = '';
      tokenInfo.style.top = '';
    });
  }
}
