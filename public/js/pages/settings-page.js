...
export class SettingsPage {
                    ...
                    <div class="settings-box">
                        ...
                        <div class="info-grid">
                            ...
                            <div class="info-item">
                                ...
                                <span id="settings-server-status" class="value">
                                    <span class="status-indicator connected" aria-label="Server connected" title="Server connected"></span>
  ...
  setupEventListeners() {
    ...
    if (saveSettings) {
      ...
      saveSettings.addEventListener('click', async (e) => {
        ...
        const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="inline-spinner"></span> Saving`;
        try { await this.handleSaveSettings(); } finally { btn.disabled = false; btn.innerHTML = backup; }
      });
    } else {
    ...
    if (testConnection) {
      testConnection.addEventListener('click', async (e) => {
        const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class=\"inline-spinner\"></span> Getting Token`;
        try { await this.handleGetToken(); } finally { btn.disabled = false; btn.innerHTML = backup; }
      });
    }
    ...
    document.getElementById('refresh-token')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="inline-spinner"></span> Getting Token`;
      try { await this.handleGetToken(); } finally { btn.disabled = false; btn.innerHTML = backup; }
    });

    document.getElementById('validate-token')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="inline-spinner"></span> Validating`;
      try { await this.handleValidateToken(); } finally { btn.disabled = false; btn.innerHTML = backup; }
    });

    document.getElementById('test-connection-token')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="inline-spinner"></span> Testing`;
      try { await this.handleTestConnection(); } finally { btn.disabled = false; btn.innerHTML = backup; }
    });

    document.getElementById('revoke-token')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class="inline-spinner"></span> Revoking`;
      try { await this.handleRevokeToken(); } finally { btn.disabled = false; btn.innerHTML = backup; }
    });

    document.getElementById('clear-token')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget; const backup = btn.innerHTML; btn.disabled = true; btn.innerHTML = `<span class=\"inline-spinner\"></span> Clearing`;
      try { await this.handleClearToken(); } finally { btn.disabled = false; btn.innerHTML = backup; }
    });
  ...
  async populateForm() {
    ...
    const preferences = {
      ...
      'show-disclaimer-startup': settings.showDisclaimerOnStartup !== false,
  ...
  /**
   * Test connection using current settings (requests a token, discards it)
   */
  async handleTestConnection() {
    try {
      this.app.showInfo('Testing connection...');
      // Use backend helper which reads settings and requests a token
      const resp = await csrfManager.fetchWithCSRF('/api/pingone/token', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } });
      const json = await resp.json().catch(() => ({}));
      if (resp.ok && json && json.success) {
        this.app.showSuccess('Connection successful. Now click "Get New Token".');
        // Refresh status after a successful test
        await this.app.loadTokenStatus();
      } else {
        const msg = json?.error || json?.message || 'Connection failed';
        this.app.showError(msg);
      }
    } catch (e) {
      this.app.showError('Test connection failed: ' + e.message);
    }
  }
}
    ...
    const checkSystemHealth = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          if (serverStatusEl) {
            const isOk = (health.status === 'ok') || (health.optimization?.overall === 'ok');
            ...
            if (indicator) {
              ...
              indicator.classList.add(isOk ? 'connected' : 'disconnected');
            }
            if (textEl) { textEl.textContent = isOk ? 'Connected' : 'Disconnected'; }
          }
