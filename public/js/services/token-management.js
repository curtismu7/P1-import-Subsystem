...
class TokenManager {
  ...
  async _requestNewToken() {
    ...
    try {
      ...
      // Emit analytics event: a token request is starting
      if (this.eventBus) {
        try { this.eventBus.emit('tokenRequest'); } catch (_) {}
      }
      ...
      if (!response.ok) {
        ...
        // Emit analytics failure event
        if (this.eventBus) {
          try { this.eventBus.emit('tokenError', { error: errorMsg }); } catch (_) {}
        }
      ...
      if (this.eventBus) {
        try {
          ...
          this.eventBus.emit('tokenObtained', { tokenType: this.tokenCache.tokenType });
        } catch (_) {}
      }
      ...
      // Emit analytics failure event
      if (this.eventBus) {
        try { this.eventBus.emit('tokenRefreshFailed', { error: error.message }); } catch (_) {}
      }
